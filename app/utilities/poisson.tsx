...
export default function PoissonDistribution() {
  ...
  React.useEffect(() => {
    import('../../src/db/database').then(({ recordActivity }) => {
      recordActivity('Poisson Calculator', 'TOOL', '/utilities/poisson');
    });
  }, []);
  ...
  const isLambdaValid = !isNaN(lambda) && lambda > 0;
  const isValid = isLambdaValid && !isNaN(x) && x >= 0;
  ...
  const runSimulation = () => {
    ...
    const { samples, tableData } = generatePoissonSamples(lambda, N);
  ...
  const chart = useMemo(() => {
    const { points, displayN } = generatePoissonChartPoints(lambda, x, modeIdx, isValid);
