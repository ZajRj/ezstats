...
export default function GeometricDistribution() {
  ...
  const runSimulation = () => {
    ...
    const { samples, tableData } = generateGeometricSamples(p, N);
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={true} style={{ backgroundColor: '#111827', borderRadius: 8, padding: 8 }}>
                <View>
                  <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#374151', paddingBottom: 8, marginBottom: 8 }}>
                    ...
                    <Text style={{ width: 200, color: '#9CA3AF', fontSize: 12, fontWeight: 'bold', textAlign: 'center' }}>X_i = ⌈ ln(1-R_i) / ln(1-p) ⌉</Text>
                  </View>
                  {simData.map((row) => (
                    <View key={row.i} style={{ flexDirection: 'row', paddingVertical: 4 }}>
                      ...
                      <Text style={{ width: 200, color: '#34D399', fontSize: 12, textAlign: 'center', fontWeight: 'bold' }}>{row.x}</Text>
                    </View>
                  ))}
                </View>
              </ScrollView>
